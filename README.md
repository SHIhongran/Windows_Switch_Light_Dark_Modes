...
### 🐛 Bug修复与UI优化
- **事件绑定修复**: 解决了因事件冲突导致无法点击修改定时切换时间的严重Bug
- **UI清晰度重构**: 优化了定时切换模块的布局和图标，使其更直观、更易用
- **交互体验提升**: 将整个时间设置区域变为可点击单元，操作更便捷

## v1.6.1 更新日志

### ✨ 功能增强
- **智能化自动切换**: 自动切换行为现在会完全尊重“切换后重启资源管理器”复选框的设置
- **配置持久化**: “重启资源管理器”选项的状态会被保存并在下次启动时自动加载

## v1.6.0 更新日志

### ✨ 功能增强
- **定时自动切换**: 新增定时切换功能，可自定义暗色/浅色模式的切换时间
- **后台调度器**: 每分钟检查一次，智能判断是否需要执行切换
- **配置持久化**: 定时切换的启用状态和时间设置会被保存并在下次启动时自动加载
